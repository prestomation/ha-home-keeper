...
test('capture NFC tag picker + chips', async ({ page }) => {
  ...
  await openPanel(page);
  await expect(panel.locator('.hk-card[data-id="task_fridge_filter"] .hk-tag')).toBeVisible({
    timeout: 10_000,
  });
  await expect(panel.locator('.hk-card[data-id="task_furnace_filter"] .hk-tag')).toBeVisible();
