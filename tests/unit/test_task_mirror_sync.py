"""Unit tests for the task-mirror driver (``task_mirror_sync.py``).
...
# What a list must have before the mirror can work at all…
...
def _load_module() -> types.ModuleType:
    """Load ``task_mirror_sync.py`` as ``hk.task_mirror_sync`` with doubles."""
    existing = sys.modules.get("hk.task_mirror_sync")
    if existing is not None:
        return existing
    _install_ha_stubs()
    ...
    spec = importlib.util.spec_from_file_location(
        "hk.task_mirror_sync", str(_COMPONENT_DIR / "task_mirror_sync.py")
    )
    assert spec and spec.loader
    module = importlib.util.module_from_spec(spec)
    sys.modules["hk.task_mirror_sync"] = module
...
task_mirror_sync = _load_module()
store_mod = _load_store()
ORIGIN = sys.modules["hk.const"].ORIGIN_TODO_MIRROR
...
class _FakeStore:
    ...
    def get_task_mirror_items(self):
        return self._mirror_items

    async def async_set_task_mirror_items(self, items):
        self.log.append("persist")
        if items == self._mirror_items:
            return False
        self._mirror_items = items
        self.writes += 1
        return True
...
def test_the_capability_map_is_derived_per_list_from_supported_features():
    ...
    store = _FakeStore(tasks={T1: _task(notes=NOTES)})
    _sync(hass, store, mirrors=[_mirror(), _mirror(M2, entity_id=OTHER)])
    assert _services(hass, "add_item") == [
...
def test_a_renamed_rescheduled_task_updates_the_item_it_is_bound_to():
    ...
    assert store.get_task_mirror_items() == _tracked()
...
def test_a_profile_selects_the_tasks_it_would_select_in_a_notification(enricher):
    # The mirror plans from tasks enriched with their **effective** labels, so a
...
def test_a_failed_update_keeps_the_entry_it_was_going_to_replace():
    ...
    assert store.get_task_mirror_items() == before  # …and not believed
...
def test_a_list_missing_a_base_feature_is_not_asked_to_use_it(features, service):
    ...
    _sync(hass, store)
    assert hass.services.calls == []
    assert store.get_task_mirror_items() == items
...
def test_a_pass_never_raises_into_the_mutation_that_triggered_it(caplog):
    ...
    store.get_task_mirror_items = _explode
...
def test_a_mirror_pointed_at_our_own_list_is_refused_once(registry, caplog):
    # Mirroring our own tasks onto our own list is a loop, and ours accepts no new
    ...
    store = _FakeStore(tasks={T1: _task()})
    with caplog.at_level("WARNING"):
        sync, _ = _sync(hass, store, mirrors=[_mirror(entity_id=OURS)])
...
def test_an_event_driven_pass_with_no_drift_reads_no_lists():
    # Every task mutation pokes this; almost none of them concern a mirror, and
...
def test_a_mirror_that_cannot_settle_says_so(caplog):
    ...
    with caplog.at_level("WARNING"):
        asyncio.run(sync.async_sync(force=True))
    assert len(passes) == task_mirror_sync._MAX_PASSES
...
def test_the_sweep_costs_nothing_until_something_is_configured_or_mirrored():
    hass = _FakeHass({LIST: []})
    idle, _ = _build(hass, _FakeStore(), mirrors=[])
    idle.async_schedule_sweep()
    assert hass.tasks == []

    # A configured mirror alone is reason enough: a task falling due mutates
    # nothing, so this tick is the only thing that notices it.
    configured, _ = _build(hass, _FakeStore(), mirrors=[_mirror()])
    configured.async_schedule_sweep()
    assert len(hass.tasks) == 1

    # …and so is bookkeeping left by a mirror since switched off, which still has
    # items to take back off a list.
    leftover, _ = _build(hass, _FakeStore(items=_tracked()), mirrors=[])
...
def _watched_lists(monkeypatch) -> list[list[str]]:
    ...
    monkeypatch.setattr(task_mirror_sync, "async_track_state_change_event", _track)
    return watched
...
def test_start_listeners_watches_every_list_and_every_task_event(monkeypatch):
    ...
    entry = _config_entry(
        mirrors=[
            _mirror(),
            _mirror(M2, entity_id=OTHER),
            _mirror("m3", entity_id=LIST),  # a second mirror on one list
            _mirror("m4", entity_id=""),  # switched off
        ]
    )
    entry.async_on_unload = unloads.append
    sync = task_mirror_sync.TaskMirrorSync(hass, entry, _FakeCoordinator(_FakeStore()))
    sync.async_start_listeners()

    assert watched == [[OTHER, LIST]]  # sorted, distinct, the off switch skipped
    assert [name for name, _ in hass.bus.listeners] == list(
        task_mirror_sync._TASK_EVENTS
    )
    # Everything the entry has to tear down: the stop flag, the state listener, and
    # one subscription per task event.
    assert len(unloads) == 2 + len(task_mirror_sync._TASK_EVENTS)


def test_no_configured_list_means_no_state_listener(monkeypatch):
    hass = _FakeHass({LIST: []})
    watched = _watched_lists(monkeypatch)
    entry = _config_entry(mirrors=[])
    sync = task_mirror_sync.TaskMirrorSync(hass, entry, _FakeCoordinator(_FakeStore()))
    sync.async_start_listeners()
    assert watched == []
...
def test_the_task_mirror_bookkeeping_round_trips_through_the_store():
    store = _fresh_store()
    assert store.get_task_mirror_items() == {}
    assert asyncio.run(store.async_set_task_mirror_items(_tracked())) is True
    assert store.get_task_mirror_items() == _tracked()
    assert store._store.saves[-1]["task_mirror_items"] == _tracked()


def test_writing_the_same_bookkeeping_again_does_not_touch_the_disk():
    # A pass runs on every task mutation and every edit to a mirrored list, and
    # most settle to exactly what was already stored.
    store = _fresh_store()
    asyncio.run(store.async_set_task_mirror_items(_tracked()))
    saves = len(store._store.saves)
    assert asyncio.run(store.async_set_task_mirror_items(_tracked())) is False
    assert len(store._store.saves) == saves


def test_a_document_written_before_task_mirrors_existed_loads_clean():
    store = _fresh_store()
...
def test_stored_task_mirror_bookkeeping_survives_a_reload():
    store = _fresh_store()
    store._store.data = {"tasks": {}, "task_mirror_items": _tracked()}
    asyncio.run(store.load())
    assert store.get_task_mirror_items() == _tracked()


def test_removing_the_integration_clears_the_task_mirror_bookkeeping():
    store = _fresh_store()
    asyncio.run(store.async_set_task_mirror_items(_tracked()))
