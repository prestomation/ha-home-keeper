...
export class HomeKeeperPanel extends HTMLElement {
   ...
   */
  private _seasonMonthEndFollow(value: Record<string, unknown>): Record<string, number> {
    const prev = (this._edit.task ?? {}) as Record<string, unknown>;
   ...
   */
  private _seasonWindow(index: number, count: number, form: HaFormElement): HTMLElement {
    ...
    if (count > 1) {
      ...
      remove.addEventListener('click', () => this._removeSeasonWindow(index));
    ...
    if (index === count && count < MAX_SEASON_WINDOWS) {
      ...
      add.addEventListener('click', () => this._addSeasonWindow());
  ...
  private _addSeasonWindow(): void {
    const count = seasonCount(this._edit.task ?? {});
    if (count >= MAX_SEASON_WINDOWS) return;
    (this._edit.task as Record<string, unknown>).season_count = count + 1;
    this._render();
  }
   ...
   */
  private _removeSeasonWindow(index: number): void {
    const count = seasonCount(this._edit.task ?? {});
    if (count <= 1) return;
    const task = this._edit.task as Record<string, unknown>;
  ...
  private _renderTaskForm(host: HTMLElement): void {
    ...
    const onChange = (value: Record<string, unknown>): void => {
        ...
        this._edit.task = {
          ...
          ...this._seasonMonthEndFollow(value),
        } as Partial<Task>;
    ...
    for (const section of sections) {
      ...
      if (seasonIndex) {
        formWrap.appendChild(
          this._seasonWindow(seasonIndex, seasonCount(task), form),
        );
