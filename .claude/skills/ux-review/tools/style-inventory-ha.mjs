// Home Keeper copy of style-inventory.mjs. It adds 2 things for Home Assistant:
// it collects the elements inside open shadow roots, and it logs in with the
// Playwright session state in $STORAGE_STATE. See ../SOURCE.md.
//
...
// Resolve playwright-core from the working directory (tests/e2e), not from this
// file, because nothing under .claude/ has a node_modules.
...
function collectStyleData() {
  ...
  // Home Assistant draws its UI in nested shadow roots. Step up through a shadow
  // root to its host, and collect the elements inside every open shadow root.
  function up(n) {
  ...
  const all = [document.body, ...deepAll(document.body, [])];
  const idOf = new Map();
  const visible = [];
  let nextId = 0;
  for (const el of all) {
...
try {
  ...
  if (inputArg.endsWith('.json')) {
    ...
    const page = await (await browser.newContext({ viewport: s.viewport || { width: 1280, height: 900 }, storageState: process.env.STORAGE_STATE || undefined })).newPage();
    ...
    const page = await (await browser.newContext({ viewport: { width: 1280, height: 900 }, storageState: process.env.STORAGE_STATE || undefined })).newPage();
    page.setDefaultTimeout(8000);
